...
MonospaceMode が有効なときは、横書きの各文字を固定セルへ並べ直します。
セル幅は 全角 = 1em、半角 = 0.5em を基本にし、各グリフはそのセル内で水平方向中央寄せになります。
...
MonospaceMode が有効な場合も、縦書きの文字送り自体は FontSize と LetterSpacing ベースです。
一方で各文字の横幅判定には同じ固定セルルールを使うため、半角文字と全角文字が混在しても列内の見え方をそろえやすくなります。
...
等幅表示モードでは、ASCII、半角カナ、半角ハングル、いくつかの半角記号を 0.5em セルとして扱います。
元文字を直接参照できない場合は、advance 幅または glyph bounds から 0.5em / 1em を推定します。
